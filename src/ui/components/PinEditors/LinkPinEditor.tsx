    ...
    <>
      ...
      <div>
        <label htmlFor="Content" className="flex flex-col">
          <span className="text-sm font-medium leading-6 text-gray-900">Link</span>
        </label>
        <div className="mt-2">
          <input
            type="text"
      ...
      <div>
        <span className="flex flex-col">
          <span className="text-sm font-medium leading-6 text-gray-900">Image</span>
          <span className="text-xs font-light text-gray-500">
            Select an option and choose a high quality image that represents your pin.
          </span>
        </span>
        <div className="mt-2">
          <ImageMenu
            image={pins && pinIndex != null ? pins[parseInt(pinIndex)]?.[2] : ''}
            setImage={(url) => {
          ...
          <div key={hIndex}>
            ...
            <div className="mt-2">
              <input
                ...
                tabIndex={hIndex + 3}
                className="block w-full rounded-md border-0 py-1.5 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-gray-600 sm:text-sm sm:leading-6"
                value={pins && pinIndex != null ? pins[parseInt(pinIndex)]?.[hIndex + 3] : ''}
                onChange={(e) => {
                  pinIndex != null && setPin(parseInt(pinIndex), hIndex + 3, e.target.value);
                }}
              />
            </div>
          </div>
